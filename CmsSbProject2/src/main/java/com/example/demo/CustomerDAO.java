...
public class CustomerDAO {
	...
	public String checkCustomer(String user) {
		String checkUserName ="select count(*) cnt from customer where Cus_UserName=?";
		List str = jdbc.query(checkUserName, new Object[] {user},new RowMapper() {
			@Override
			public Object mapRow(ResultSet rs,int rowNum) throws SQLException{
				return rs.getInt("cnt");
			}
	...
	public Customer searchByCustomerUserId(int cid) {
		String cmd = "select * from Customer where Cus_ID=?";
		List<Customer> customerList = jdbc.query(cmd, new Object[] {cid}, new RowMapper<Customer>() {
