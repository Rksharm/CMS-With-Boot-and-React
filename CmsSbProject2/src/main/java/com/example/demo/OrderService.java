...
public class OrderService {
	...
	public String acceptOrRejectOrder(int ordId,int venId,String status) {
		...
		Customer customer = cdao.searchByCustomerUserId(cid);
		String reciever= customer.getCusEmail();
		...
		if (status.toUpperCase().equals("YES")) {
			email.setRecipient(reciever);
			email.setSubject("Order status");
			email.setMsgBody("your order has been accepted");
			System.out.println(edao.sendMail(email));
